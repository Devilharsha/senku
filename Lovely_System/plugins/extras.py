...
async def join(event) -> None:
    ...
    if private:
        ...
        await System.send_message(
            Lovely_logs,
            f"{(await event.get_sender()).first_name} made SENKU join {private.group(5)}",
        )
    else:
        ...
        await System.send_message(
            Lovely_logs,
            f"{(await event.get_sender()).first_name} made SENKU join {link}",
        )
...
async def leave(event) -> None:
    ...
    if c_id:
        ...
        await System.send_message(
            event.chat_id, f"SENKU has left chat with id[-{c_id.group(1)}]"
        )
    else:
        await System(LeaveChannelRequest(link))
        await System.send_message(event.chat_id, f"Senku has left chat[{link}]")
